...
const checkError = (username,password,name,jobseekerid) => {
	var errorCheck = ['','','','']
	var counter = 0;
	if(username === ""){
		...
		request.post('http://localhost:3001/employees/AddUser-JobSeeker', {form: {"username":username,"password":password,"name":name,"jobseekerid":jobseekerid+1}},(error, response, body) => {
			console.log(body);
			if(body) {
...
class SignUpForm extends Component {
	...
	handleButtonChange(e) {
		this.setState({errorCheck: checkError(this.state.username,this.state.password,this.state.name,this.state.jobseekerid)});
	}

	componentDidMount() {
		fetch('http://localhost:3001/jobSeekerUserID')
		.then((reponse) => { return reponse.json() })
		.then((result) => {
			this.setState({jobseekerid:result[0].jobseekerid});
			console.log(this.state.jobseekerid);
		})
	}
