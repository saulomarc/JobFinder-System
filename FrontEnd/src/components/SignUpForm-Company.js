...
const checkError = (username,password,name,userid) => {
	var errorCheck = ['','','','','']
	var counter = 0;
	if(username === ""){
		...
		errorCheck[4] = "  Account Successfully Created!";
		request.post('http://localhost:3001/employees/AddUser-Company', {form: {"username":username,"password":password,"name":name,"userid":userid+1}},(error, response, body) => {
			console.log(body);
			if(body) {
...
class SignUpForm extends Component {
	...
	componentDidMount() {
		fetch('http://localhost:3001/companyUserID')
		.then((reponse) => { return reponse.json() })
		.then((result) => {
			this.setState({userid:result[0].cuserid});
	...
	handleButtonChange(e) {
		this.setState({errorCheck: checkError(this.state.username,this.state.password,this.state.name,this.state.userid)});
	}
